...
def _make_mpf_addplot_overlay(ohlc_df: pd.DataFrame, value_line: pd.Series, num_candles: int):
    ...
    ohlc_df = ohlc_df.dropna(subset=['open'])
    # ohlc_df = ohlc_df.tail(num_candles)
    ohlc_df = ohlc_df[-num_candles:]
...
def plot_charts(symbols:list, timeframe:str, num_candles:int=200, include_forecasts:bool=True):
    ...
    for symbol in symbols:
        if symbol in indexes:
            ...
            overlay = _make_mpf_addplot_overlay(ohlc, value_lines[symbol], num_candles=num_candles)
            try:
...
if __name__ == '__main__':
    ...
    plot_charts(indexes, '8H', include_forecasts=True, num_candles=200)
